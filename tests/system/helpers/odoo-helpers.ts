...
export async function testOdooConnection(): Promise<TestOdooConnection> {
  try {
    ...
    if (status.isConnected) {
      ...
      return {
        isConnected: true,
        database: status.database || ODOO_CONFIG.database,
 ...
 */
export function validateDataStructure(data: any, expectedFields: string[]): TestDataValidation {
  const errors: string[] = [];
...
export function generateOdooTestReport(testResults: any[]) {
  ...
  if (responseTimes.length > 0) {
    report.summary.avgResponseTime = responseTimes.reduce((a, b) => a + b, 0) / responseTimes.length;
  }
