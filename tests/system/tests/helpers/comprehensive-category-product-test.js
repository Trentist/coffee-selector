...
/**
 * Comprehensive Category & Product Test - اختبار شامل للفئات والمنتجات
 * اختبار استعلامات الفئات والمنتج الواحد بالـ ID
 */
...
// ============================================================================
// CATEGORY QUERIES - استعلامات الفئات
// ============================================================================
		...
		if (result.data?.categories?.categories) {
			...
			categories.forEach((category, index) => {
				...
				if (category.children && category.children.length > 0) {
					console.log(`      👶 الأقسام الفرعية: ${category.children.length}`);
					category.children.slice(0, 3).forEach((child) => {
						console.log(`         - ${child.name}`);
					});
					if (category.children.length > 3) {
						console.log(`         ... و ${category.children.length - 3} أكثر`);
					}
				}
			});
	...
	try {
		...
		if (result.data?.category) {
			...
			if (category.description) {
				console.log(`📝 الوصف: ${category.description.substring(0, 100)}...`);
			}
			...
			if (category.children && category.children.length > 0) {
				console.log(`👶 الأقسام الفرعية: ${category.children.length}`);
				category.children.slice(0, 3).forEach((child, index) => {
...
async function testGetProductById(productId) {
	...
	try {
		...
		if (result.data?.product) {
			...
			console.log(`🆔 SKU: ${product.sku || "غير محدد"}`);
			console.log(`📦 متوفر: ${product.isInStock ? "نعم" : "لا"}`);
			console.log(`📊 الكمية: ${product.quantity || "غير محدد"}`);
			...
			if (product.shortDescription) {
				console.log(
					`📝 الوصف المختصر: ${product.shortDescription.substring(0, 80)}...`,
				);
			}
			...
			if (product.attributes && product.attributes.length > 0) {
				console.log(`🔧 الخصائص:`);
				product.attributes.slice(0, 5).forEach((attr, index) => {
					console.log(`   ${index + 1}. ${attr.name}: ${attr.value}`);
				});
			}

			if (product.reviews && product.reviews.length > 0) {
				console.log(`⭐ التقييمات: ${product.reviews.length} تقييم`);
				console.log(`📊 متوسط التقييم: ${product.averageRating || "غير محدد"}`);
			}

			if (product.gallery && product.gallery.length > 0) {
				console.log(`🖼️  معرض الصور: ${product.gallery.length} صورة`);
			}
...
// Test 5: Get Products by Category ID
async function testGetProductsByCategoryId(categoryId) {
	console.log(`\n🛍️ اختبار 5: الحصول على منتجات الفئة بالـ ID (${categoryId})`);
	console.log("=".repeat(50));
	...
	try {
		const result = await makeGraphQLRequest(query, { categoryId: categoryId });

		if (result.data?.categoryProducts?.products) {
			const products = result.data.categoryProducts.products;
			const totalCount = result.data.categoryProducts.totalCount;
			...
			if (products.length > 0) {
				...
				products.slice(0, 5).forEach((product, index) => {
					...
					console.log(`      📦 متوفر: ${product.isInStock ? "نعم" : "لا"}`);
					console.log(
						`      ⭐ التقييم: ${product.averageRating || "غير محدد"}`,
					);
					console.log(`      📝 عدد التقييمات: ${product.reviewCount || 0}`);
				});
...
async function runComprehensiveCategoryProductTest() {
	console.log("🚀 اختبار شامل للفئات والمنتجات");
	...
	try {
		...
		// Test 5: Get Products by Category ID (if we have a sample category)
		if (sampleCategoryId) {
			results.tests.productsByCategory =
				await testGetProductsByCategoryId(sampleCategoryId);
		} else {
...
if (require.main === module) {
	runComprehensiveCategoryProductTest()
		.then((results) => {
			console.log("\n✅ اكتمل الاختبار الشامل للفئات والمنتجات بنجاح!");
...
module.exports = { runComprehensiveCategoryProductTest };
