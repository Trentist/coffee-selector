...
/**
 * Aramex Shipment Setup Test - Complete Form Variables & Auto Response
 * اختبار إعداد شحنة أرامكس - متغيرات الفورم الكاملة والاستجابة التلقائية
 *
 ...
 * 4. Integration with quotation system
 */
...
async function createCompleteQuotation() {
	...
	const shippingCost = calculateShippingCost(
		totalWeight,
		quotationData.shipping.serviceType,
	);
	...
	console.log(
		`   💰 المجموع الفرعي: ${completeQuotation.financial.subtotal} ${completeQuotation.financial.currency}`,
	);
	console.log(
		`   🚚 تكلفة الشحن: ${completeQuotation.shipping.cost} ${completeQuotation.financial.currency}`,
	);
	console.log(
		`   💸 الضريبة: ${completeQuotation.financial.tax} ${completeQuotation.financial.currency}`,
	);
	console.log(
		`   💳 الإجمالي النهائي: ${completeQuotation.financial.total} ${completeQuotation.financial.currency}`,
	);
...
/**
 * Step 2: Send Quotation to Odoo and Get Order ID
 * الخطوة الثانية: إرسال الكوتيشن إلى أودو والحصول على معرف الطلب
 */
async function sendQuotationToOdoo(quotation) {
	console.log("\n🔄 الخطوة 2: إرسال الكوتيشن إلى أودو");
	console.log("=".repeat(70));

	// First, let's get available products to use their IDs
	const getProductsQuery = `
    query GetProductsForQuotation {
      products {
        products {
          id
          name
  ...
  `;
	...
	let productIds = [1, 2]; // Default IDs

	if (productsResult.data?.products?.products) {
		productIds = productsResult.data.products.products
			.slice(0, 2)
			.map((p) => p.id);
	}

	const createQuotationMutation = `
  ...
  `;
	...
	const quotationInput = {
		...
		items: quotation.products.map((product, index) => ({
			product_id: productIds[index] || product.id,
			quantity: product.quantity,
			price_unit: product.unitPrice,
		})),
		notes: quotation.order.notes,
		valid_until: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(), // 30 days
	};

	try {
		const result = await makeGraphQLRequest(createQuotationMutation, {
			input: quotationInput,
		});

		if (result.data?.createQuotation?.success) {
			const odooQuotation = result.data.createQuotation.quotation;

			console.log("✅ تم إنشاء الكوتيشن في أودو بنجاح:");
			console.log(`   🆔 معرف الكوتيشن: ${odooQuotation.id}`);
			console.log(`   📝 اسم الكوتيشن: ${odooQuotation.name}`);
			console.log(`   📊 الحالة: ${odooQuotation.state}`);
			console.log(`   💰 الإجمالي: ${odooQuotation.amount_total}`);
			console.log(`   📦 عدد العناصر: ${odooQuotation.items.length}`);

			return {
				success: true,
				quotationId: odooQuotation.id,
				quotationName: odooQuotation.name,
				odooQuotation: odooQuotation,
			};
		} else {
			console.log("❌ فشل في إنشاء الكوتيشن في أودو");
			if (result.errors) {
		...
		console.log(`❌ خطأ في إرسال الكوتيشن: ${error.message}`);
 ...
 */
async function simulateAramexAPICall(quotation, orderId) {
	...
	console.log(
		`   💰 التكلفة: ${aramexResponse.cost.amount} ${aramexResponse.cost.currency}`,
	);
	console.log(
		`   📅 تاريخ التسليم المتوقع: ${new Date(aramexResponse.estimatedDelivery).toLocaleDateString("ar-SA")}`,
	);
...
/**
 * Step 4: Store Label URL in Odoo using setAramexLabelUrl Mutation
 * الخطوة الرابعة: تخزين رابط التتبع في أودو باستخدام mutation setAramexLabelUrl
 */
async function storeAramexLabelUrl(orderId, aramexResponse) {
	console.log("\n💾 الخطوة 4: تخزين رابط التتبع في أودو");
	console.log("=".repeat(70));
  ...
  `;
	...
	try {
		console.log(`🔄 تخزين رابط التتبع للطلب ${orderId}:`);
		console.log(`   🏷️ رابط التتبع: ${aramexResponse.labelUrl}`);
		...
		if (result.data?.setAramexLabelUrl?.success) {
			...
			console.log(
				`   📍 رقم التتبع: ${updatedOrder.trackingNumber || "غير محدد"}`,
			);
			console.log(
				`   🚚 حالة الشحن: ${updatedOrder.shippingStatus || "قيد المعالجة"}`,
			);
			console.log(
				`   📦 حالة التوصيل: ${updatedOrder.deliveryStatus || "قيد التوصيل"}`,
			);
 ...
 */
async function createShipmentSummary(
	quotation,
	orderId,
	aramexResponse,
	storedOrder,
	...
	console.log(`\n📋 معلومات الكوتيشن:`);
	console.log(`   🆔 المعرف: ${shipmentSummary.quotation.id}`);
	console.log(`   🔗 المرجع: ${shipmentSummary.quotation.reference}`);
	console.log(`   👤 العميل: ${shipmentSummary.quotation.customer.name}`);
	console.log(
		`   💰 الإجمالي: ${shipmentSummary.quotation.total} ${shipmentSummary.quotation.currency}`,
	);
	...
	console.log(
		`   💰 التكلفة: ${shipmentSummary.aramex.cost.amount} ${shipmentSummary.aramex.cost.currency}`,
	);
	console.log(
		`   📅 تاريخ التسليم: ${new Date(shipmentSummary.aramex.estimatedDelivery).toLocaleDateString("ar-SA")}`,
	);
	...
	console.log(
		`   🏷️ رابط التتبع المخزن: ${shipmentSummary.odoo.storedLabelUrl}`,
	);
	console.log(
		`   📍 رقم التتبع المخزن: ${shipmentSummary.odoo.storedTrackingNumber}`,
	);
	...
	console.log(
		`   🕐 الوقت: ${new Date(shipmentSummary.integration.timestamp).toLocaleString("ar-SA")}`,
	);
...
function calculateShippingCost(weight, serviceType) {
	...
	const baseCost = 25.0;
	const weightCost = weight * 5.0; // 5 AED per kg
 ...
 */
async function runAramexShipmentSetupTest() {
	console.log("🚀 اختبار إعداد شحنة أرامكس الكامل");
	...
	try {
		...
		// Step 2: Send to Odoo
		results.steps.odoo = await sendQuotationToOdoo(
			results.steps.quotation.quotation,
		);
		if (!results.steps.odoo.success) {
			throw new Error("فشل في إرسال الكوتيشن إلى أودو");
		}

		// Step 3: Simulate Aramex API
		results.steps.aramex = await simulateAramexAPICall(
			results.steps.quotation.quotation,
			results.steps.odoo.quotationId,
		);
		if (!results.steps.aramex.success) {
			throw new Error("فشل في استدعاء API أرامكس");
		}

		// Step 4: Store Label URL
		results.steps.storage = await storeAramexLabelUrl(
			results.steps.odoo.quotationId,
			results.steps.aramex.aramexResponse,
		);
		if (!results.steps.storage.success) {
			throw new Error("فشل في تخزين رابط التتبع");
		}
		...
		results.steps.summary = await createShipmentSummary(
			...
			results.steps.odoo.quotationId,
			results.steps.aramex.aramexResponse,
			results.steps.storage.order,
		);

		// Final Results
		console.log("\n" + "=".repeat(80));
		console.log("🎉 نتائج اختبار إعداد شحنة أرامكس الكامل");
		console.log("=".repeat(80));

		const successRate = Object.values(results.steps).filter(
			(step) => step.success,
		).length;
		...
		console.log(
			`✅ معدل النجاح: ${percentage}% (${successRate}/${totalSteps})`,
		);
		console.log(`🆔 معرف الطلب: ${results.steps.odoo.quotationId}`);
		console.log(
			`🏷️ رابط التتبع: ${results.steps.aramex.aramexResponse.labelUrl}`,
		);
		console.log(
			`📍 رقم التتبع: ${results.steps.aramex.aramexResponse.trackingNumber}`,
		);
		...
		console.log(`   🏷️ رابط التتبع: ✅ مخزن تلقائياً`);
		console.log(`   🔗 التكامل مع الكوتيشن: ✅ مكتمل`);

		console.log("\n📋 الاستجابة التلقائية:");
		console.log(`   🚚 استدعاء أرامكس: ✅ تم بنجاح`);
		console.log(`   💾 تخزين الرابط: ✅ تم تلقائياً`);
		console.log(`   🔄 تحديث الطلب: ✅ تم في أودو`);
		console.log(`   📊 التكامل الكامل: ✅ مكتمل`);

		results.finalSummary = results.steps.summary.summary;
		results.successRate = percentage;
		results.overallSuccess = successRate === totalSteps;
